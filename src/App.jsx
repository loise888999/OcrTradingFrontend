...
const zoneNames = { coordinate: 'Coordinate', city: 'City', price: 'Price' };
  ...
  return (
    <Card className="map-card full-map-card">
      <div className="card-header dark-header compact-header">
        <div>
          <h2><Map size={22} /> Coordinate Map</h2>
          <p>World size: {worldWidth} x {worldHeight}. Image relationship: 4096 x 2048 scaled by 4.</p>
        </div>
        <div className="button-row">
          <Button variant="secondary" onClick={() => setZoom((z) => Math.max(0.018, z / 1.14))}>Zoom out</Button>
          <Button variant="secondary" onClick={() => setZoom((z) => Math.min(0.75, z * 1.14))}>Zoom in</Button>
...
function PricesTab({ prices, refreshPrices }) {
  ...
  const columns = [
    { key: 'city', label: 'City', sortable: true, render: (row) => sanitizeCityName(row.city) },
    { key: 'itemName', label: 'Item', sortable: true },
    { key: 'tradeGoodType', label: 'Type', sortable: true },
    { key: 'tradeType', label: 'Trade', sortable: true, render: (row) => <Badge tone={row.tradeType === 'Buy' ? 'success' : row.tradeType === 'Sell' ? 'info' : 'muted'}>{row.tradeType}</Badge> },
    { key: 'price', label: 'Price', sortable: true },
    { key: 'multiplier', label: 'Multiplier', sortable: true, render: (row) => row.multiplier == null ? 'Missing' : `${Number(row.multiplier).toFixed(0)}%` },
    { key: 'capturedAtUtc', label: 'Captured', sortable: true, render: (row) => formatDate(row.capturedAtUtc) }
  ];
  ...
  return (
    <Card>
      <div className="card-body">
        ...
        <SortableTable columns={columns} rows={rows} emptyMessage="No price history yet." initialSortKey="capturedAtUtc" initialDirection="desc" />
      </div>
    </Card>
  );
}
...
export default function App() {
  ...
  const [settings, setSettings] = useState({ worldWidth: DEFAULT_WORLD_WIDTH, worldHeight: DEFAULT_WORLD_HEIGHT, xZeroOffset: DEFAULT_X_ZERO_OFFSET, waypointOffsetX: DEFAULT_WAYPOINT_OFFSET_X, waypointOffsetY: DEFAULT_WAYPOINT_OFFSET_Y, ocrInterval: DEFAULT_OCR_INTERVAL, cityInterval: DEFAULT_CITY_INTERVAL });

  const run = useCallback(async (fn, fallbackMessage = 'Request failed') => {
    try { setError(''); return await fn(); }
    catch (err) { setError(err?.message || fallbackMessage); return null; }
  }, []);
  ...
  const refreshCoordinates = useCallback(async () => { const data = await run(() => api.getLatestCoordinates({ take: 20 }), 'Could not load coordinates'); if (data) setCoordinates(data); }, [run]);
  const refreshPrices = useCallback(async () => { const data = await run(() => api.getPriceHistory({ take: 500 }), 'Could not load prices'); if (data) setPrices(data); }, [run]);
  const refreshSettings = useCallback(async () => { const data = await run(() => api.getSettings(), 'Could not load settings'); if (data?.zones) setZones(data.zones); }, [run]);
  const refreshCatalogs = useCallback(async () => { const loadedCities = await run(() => api.getCities(), 'Could not load cities'); if (loadedCities) setCities(loadedCities); const loadedGoods = await run(() => api.getTradeGoods(), 'Could not load trade goods'); if (loadedGoods) setTradeGoods(loadedGoods); }, [run]);

  const refreshAll = useCallback(async () => Promise.all([refreshStatus(), refreshCoordinates(), refreshPrices(), refreshSettings(), refreshCatalogs()]), [refreshStatus, refreshCoordinates, refreshPrices, refreshSettings, refreshCatalogs]);

  useEffect(() => {
    refreshAll();
    const timer = setInterval(() => { refreshStatus(); refreshCoordinates(); refreshPrices(); }, Math.max(1, settings.ocrInterval) * 1000);
    return () => clearInterval(timer);
  }, [refreshAll, refreshStatus, refreshCoordinates, refreshPrices, settings.ocrInterval]);

  const saveZone = async (zone) => { const saved = await run(() => api.saveOcrZone(zone), 'Could not save OCR zone'); if (saved) await refreshSettings(); };
  const saveSetting = async (setting) => { await run(() => api.saveSetting(setting), 'Could not save setting'); };
  const startOcr = async () => { await run(() => api.startOcr(), 'Could not start OCR'); await refreshStatus(); };
  const stopOcr = async () => { await run(() => api.stopOcr(), 'Could not stop OCR'); await refreshStatus(); };

  return (
    <div className="app-shell">
      <header className="hero"><div><h1>OCR Trading Companion</h1><p>Window-relative OCR zones, OCR quick control, split trading tabs, price sharing, and route recommendations.</p></div><Badge tone="info">React + C# Backend</Badge></header>
      <StatusBar backendStatus={backendStatus} ocrStatus={ocrStatus} latestCity={latestCity} error={error} onRefresh={refreshAll} startOcr={startOcr} stopOcr={stopOcr} refreshStatus={refreshStatus} />
      <nav className="tabs">
